...
def initialize_network():
	...
	for channel_1 in [22, 23, 24, 25, 26]:
		...
		for i in range(1, len(lines)-1):
		    ...
		    if i!=1:
		    	rssi_strength = rssi_strength+SPACE_RSSI_REDUCE
		    for node1 in nodes_pre:
		        ...
		        for node2 in nodes_post:
		            if i<len(lines)-2:
		                ...
		                #print "( "+str(node1)+", "+str(node2)+" )"+" rssi_level: "+str(float(rssi_strength))
		                #print "( "+str(node2)+", "+str(node1)+" )"+" rssi_level: "+str(float(rssi_strength))
		            else:
	...
	#print "return to initialize_network"
	return len(topo)
	#
...
server_address = ('localhost', 10036)
